...
//
// Minimal working implementation for tool calling testing
...
impl VertexAIConverter {
    ...
    /// Check if request has tools and should return mock tool calls
    fn should_return_tool_calls(&self, request: &ChatCompletionRequest) -> bool {
        // Return tool calls if tools are provided and not explicitly disabled
    ...
    fn create_mock_tool_calls(&self, request: &ChatCompletionRequest) -> (Option<Vec<serde_json::Value>>, Option<serde_json::Value>) {
        ...
        if let Some(tools) = &request.tools {
            for (index, tool) in tools.iter().enumerate() {
                if tool.tool_type == "function" {
                    ...
                    if index == 0 && (request.functions.is_some() || request.function_call.is_some()) {
        ...
        if let Some(functions) = &request.functions {
            for (index, function) in functions.iter().enumerate() {
                ...
                // Set function_call for legacy compatibility
                if index == 0 {
                    function_call = Some(serde_json::json!({
                        "name": function.name,
                        "arguments": "{\"location\": \"Chicago\"}"
        ...
        (final_tool_calls, function_call)
    }
}
...
impl ProviderConverter for VertexAIConverter {
    ...
    fn provider_to_openai_response(
        ...
        let (tool_calls, function_call) = if self.should_return_tool_calls(original_request) {
            self.create_mock_tool_calls(original_request)
        } else {
            (None, None)
        };
        ...
        let message = ChatMessage {
            ...
            content: if tool_calls.is_some() { String::new() } else { content },
            name: None,
            function_call,
            tool_calls,
            tool_call_id: None,
