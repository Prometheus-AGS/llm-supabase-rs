...
impl GroqClient {
    ...
    pub async fn new(config: GroqConfig) -> Result<Self> {
        ...
        config.validate()
            .context("Invalid Groq configuration")?;
        ...
        let client = Client::builder()
            ...
            .user_agent(&config.user_agent.as_ref().unwrap_or(&"LLM-Supabase-RS/1.0 (Groq)".to_string()))
            .build()
        ...
        info!("Successfully initialized Groq client with API key: {} for model: {}", 
              auth.get_masked_api_key(), config.model);
    ...
    pub async fn from_env() -> Result<Self> {
        let config = GroqConfig::from_env()
            .context("Failed to load Groq configuration from environment")?;
    ...
    pub async fn chat_completion(
        ...
        debug!("Making Groq chat completion request to model: {}", request.model);
        ...
        self.converter.optimize_for_groq(&mut request, &request.model)?;
        ...
        if let Some(rate_limit) = GroqStreamUtils::extract_rate_limit_from_headers(response.headers()) {
            debug!("Groq rate limit: {}/{} remaining, resets in {}s", 
                   rate_limit.remaining, rate_limit.limit, rate_limit.seconds_until_reset());
        }
        ...
        debug!("Successfully received Groq chat completion response with {} choices in {:?}", 
               chat_response.choices.len(), elapsed);
    ...
    pub async fn chat_completion_stream(
        ...
        debug!("Making Groq streaming chat completion request to model: {}", request.model);

        // Validate model supports streaming
        if !GroqModel::get_model(&request.model)
            .map(|m| m.supports_streaming)
            .unwrap_or(false) {
            return Err(anyhow::anyhow!("Model '{}' does not support streaming", request.model));
        }
    ...
    pub fn process_tool_results(&self, results: Vec<ToolCallResult>) -> Result<Vec<crate::models::common::ChatMessage>> {
    ...
    fn update_performance_metrics(&self, elapsed: Duration) {
        ...
        self.total_response_time.fetch_add(elapsed.as_millis() as u64, Ordering::Relaxed);
    }
...
mod tests {
    use super::*;
    use crate::models::common::{MessageRole, ChatMessage};
